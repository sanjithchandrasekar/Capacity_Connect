    ...
    <TrainerLayout>
      <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-5xl mx-auto space-y-6">
        <motion.div variants={fadeUp} className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-zinc-200">My Courses</h2>
            <p className="text-zinc-200/60 text-sm mt-1">{courses.length} course{courses.length !== 1 ? 's' : ''} total</p>
          </div>
        ...
        {loading ? (
          ...
          <motion.div variants={fadeUp}>
            <Card className="bg-[#070E20]/90 border-cyan-500/30">
              <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                <p className="text-zinc-200/60 mb-4">No courses yet.</p>
                <Link to="/trainer/courses/new">
            <Button className="bg-ink hover:bg-ink/90 text-cream">
          ...
          <motion.div variants={fadeUp} className="space-y-3">
            {courses.map(course => (
              <div key={course.id} className="p-5 rounded-xl bg-[#040814] border border-cyan-500/30 hover:border-cyan-500/30 transition-all">
                <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                  {/* Thumbnail */}
                  <div className="w-full sm:w-32 h-24 rounded-lg bg-ink/10 border border-cyan-500/30 overflow-hidden shrink-0">
                    <Thumbnail path={course.thumbnail_path} alt={course.title} fallbackIcon={<BookOpen className="w-8 h-8 text-zinc-200/30" />} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <Link to={`/trainer/courses/${course.id}`} className="hover:underline min-w-0 flex-1">
                        <h3 className="text-base font-semibold text-zinc-200 truncate">{course.title}</h3>
                      </Link>
                      <Badge className={`shrink-0 text-[10px] ${statusColors[course.status] ?? statusColors.draft}`}>
                        {statusLabels[course.status] ?? course.status}
                      </Badge>
                    </div>
                    <p className="text-sm text-zinc-200/60 line-clamp-2 mb-3">{course.description}</p>
                    <div className="flex items-center gap-4 text-xs text-zinc-200/50">
                  ...
                  <div className="flex flex-wrap gap-2 shrink-0">
                    <Button 
                      size="sm" 
                      variant="outline" 
                      className="border-cyan-500/30 text-zinc-200 hover:bg-ink/5"
