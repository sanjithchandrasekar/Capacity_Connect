...
export function TrainerLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      <div className="h-14 flex items-center px-4 border-b border-cyan-500/30">
      ...
      <div className="p-3 border-t border-cyan-500/30 space-y-3">
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm text-zinc-200 font-medium truncate">{profile?.full_name}</p>
            <p className="text-xs text-zinc-200/50 truncate">{profile?.email}</p>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#070E20]/90 text-zinc-200 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-cyan-500/30 bg-[#070E20]/90/95 backdrop-blur-2xl flex flex-col transition-transform duration-300 md:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="absolute top-3 right-3 z-10">
          <button onClick={() => setMobileOpen(false)} className="p-2 rounded-lg hover:bg-cyan-950/30 text-zinc-200/50 hover:text-zinc-200 transition-all">
            <X className="w-5 h-5" />
          </button>
        </div>
        {sidebarContent}
      ...
      <aside className="relative z-20 hidden md:flex w-64 border-r border-cyan-500/30 bg-[#070E20]/90/80 backdrop-blur-2xl flex-col shrink-0 h-screen sticky top-0">
        {sidebarContent}
      </aside>

      <div className="flex-1 min-w-0">
        <header className="h-14 border-b border-cyan-500/30 bg-[#070E20]/90/80 backdrop-blur-2xl sticky top-0 z-10 flex items-center px-4 md:px-6 gap-3">
          <button onClick={() => setMobileOpen(true)} className="p-2 rounded-lg hover:bg-cyan-950/30 text-zinc-200/50 hover:text-zinc-200 transition-all md:hidden">
            <Menu className="w-5 h-5" />
          </button>
          <div className="flex items-center gap-2">
            <GraduationCap className="w-5 h-5 text-purple-600" />
            <span className="text-sm font-semibold text-zinc-200">Trainer Dashboard</span>
          </div>
          <div className="flex-1" />
          <div className="flex items-center gap-1 md:gap-2">
            <Link to="/trainer/notifications" className="relative p-2 rounded-lg hover:bg-cyan-950/30 text-zinc-200/50 hover:text-zinc-200 transition-all">
              <Bell className="w-4 h-4" />
              {unreadCount !== null && unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-pink-500 rounded-full" />
              )}
            </Link>
            <Link to="/trainer/settings" className="p-2 rounded-lg hover:bg-cyan-950/30 text-zinc-200/50 hover:text-zinc-200 transition-all">
