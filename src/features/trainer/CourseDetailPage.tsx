...
const statusColors: Record<string, string> = {
  ...
  published: 'bg-ink/10 text-zinc-200/70 border border-cyan-500/30',
    ...
    return (
      <TrainerLayout>
        <div className="max-w-3xl mx-auto py-20 text-center">
          <p className="text-zinc-200/60 mb-4">Course not found or you don't have access.</p>
          <RouterLink to="/trainer/courses"><Button variant="outline" className="border-cyan-500/30 text-zinc-200">Back to Courses</Button></RouterLink>
        </div>
      </TrainerLayout>
    )
  }
    ...
    <TrainerLayout>
      <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-4xl mx-auto space-y-6">
        <motion.div variants={fadeUp}>
          <RouterLink to="/trainer/courses" className="flex items-center gap-2 text-sm text-zinc-200/60 hover:text-zinc-200 transition-colors mb-4">
            <ArrowLeft className="w-4 h-4" /> Back to Courses
          </RouterLink>
        </motion.div>

        <motion.div variants={fadeUp} className="p-6 rounded-2xl bg-[#040814] border border-cyan-500/30 relative overflow-hidden">
          <div className="relative z-10">
            <div className="flex items-start gap-4 mb-4">
              {/* Thumbnail */}
              {course.thumbnail_path && (
                <div className="w-24 h-18 rounded-lg bg-ink/10 border border-cyan-500/30 overflow-hidden shrink-0">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-zinc-200/60 mb-4">{course.description}</p>
                <div className="flex flex-wrap items-center gap-4 text-xs text-zinc-200/50 mb-4">
            ...
            <div className="mb-4">
                      ...
                      <div className="space-y-3">
                        {pendingEnrollments.length > 0 ? (
                          pendingEnrollments.map((enrollment) => (
                            <div key={enrollment.id} className="p-4 rounded-xl bg-[#070E20]/90 border border-orange-100 flex items-center justify-between">
                              <div>
                                <h4 className="text-sm font-semibold text-zinc-200">{enrollment.trainee?.full_name || 'Unknown Trainee'}</h4>
                                <p className="text-xs text-zinc-200/60">{enrollment.trainee?.email}</p>
                              </div>
                  ...
                  <Card className="bg-[#070E20]/90 border-cyan-500/30">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        {course?.max_trainees && (
                          <span className="text-xs font-semibold px-2.5 py-1 bg-ink/5 text-zinc-200/70 rounded-full border border-cyan-500/30">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div className="bg-ink/5 border border-cyan-500/30 rounded-lg p-3 text-center">
                          <span className="block text-2xl font-bold text-zinc-200/70">{allEnrollments.filter(e => e.status === 'withdrawn').length}</span>
                          <span className="text-[10px] uppercase font-bold text-zinc-200/60 tracking-wider">Withdrawn</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                        {historyEnrollments.length > 0 ? (
                            ...
                            <div key={enrollment.id} className="p-3 rounded-xl bg-ink/5 flex items-center justify-between">
                              ...
                              <div>
                                <Badge variant="outline" className={`text-[10px] capitalize ${
                                  ...
                                  'border-cyan-500/30 text-zinc-200/60 bg-ink/5'
                                }`}>
                          ...
                          <p className="text-xs text-zinc-200/50 text-center py-4">No historical enrollments yet.</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
                ...
                {(course.start_date || course.end_date || course.meet_link || course.live_class_timing || course.mock_test_timing || course.final_exam_timing) && (
                  <div className="bg-ink/5 p-4 rounded-lg space-y-3 mt-4">
                    <h4 className="text-xs font-bold text-zinc-200 flex items-center gap-2"><Calendar className="w-4 h-4" /> Schedule & Dates</h4>
                    {(course.start_date || course.end_date) && (
                      <div className="flex items-center gap-2 text-xs text-zinc-200/70">
                        <span className="font-medium text-zinc-200 w-24">Course Span:</span> 
                    ...
                    {course.meet_link && (
                      <div className="flex items-center gap-2 text-xs text-zinc-200/70">
                        <span className="font-medium text-zinc-200 w-24">Meeting Link:</span> 
                ...
                {((course.planned_assessments_count || 0) > 0 || (course.planned_mock_tests_count || 0) > 0 || course.final_test_date) && (
                  <Card className="bg-[#070E20]/90 border-cyan-500/30 mt-4">
                    <CardContent className="p-6 space-y-4">
                      <h3 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
                        <Target className="w-4 h-4 text-zinc-200/60" /> Test & Assessment Plan
                      </h3>
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        {(course.planned_assessments_count || 0) > 0 && (
                          <div>
                            <span className="text-zinc-200/60 block text-xs">Daily Assessments</span>
                            <span className="text-zinc-200 font-medium">{course.planned_assessments_count} Planned</span>
                          </div>
                        )}
                        ...
                        {(course.planned_mock_tests_count || 0) > 0 && (
                          <div>
                            ...
                            <span className="text-zinc-200 font-medium">{course.planned_mock_tests_count} Planned</span>
                          </div>
                        )}
        ...
        {(course.session_flow_text || course.session_flow_document_path) && (
          <motion.div variants={fadeUp}>
            <Card className="bg-[#070E20]/90 border-cyan-500/30">
              <CardContent className="p-6 space-y-4">
                <h3 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
                  <BookOpen className="w-4 h-4 text-zinc-200/60" /> Session Flow
                </h3>
                <div className="space-y-3">
                  {course.session_flow_text && (
                    <p className="text-xs text-zinc-200/80 whitespace-pre-wrap">{course.session_flow_text}</p>
                  )}
          ...
          <motion.div variants={fadeUp}>
            <Card className="bg-[#070E20]/90 border-cyan-500/30">
              <CardContent className="p-6 space-y-3">
                ...
                {objectives.understand && <div><p className="text-xs text-zinc-200 font-medium">Understand</p><p className="text-sm text-zinc-200/80">{objectives.understand}</p></div>}
                {objectives.able_to_do && <div><p className="text-xs text-zinc-200 font-medium">Able to Do</p><p className="text-sm text-zinc-200/80">{objectives.able_to_do}</p></div>}
                {objectives.competencies_built && <div><p className="text-xs text-zinc-200 font-medium">Competencies Built</p><p className="text-sm text-zinc-200/80">{objectives.competencies_built}</p></div>}
              </CardContent>
            </Card>
          </motion.div>
        )}
        ...
        {courseSkills.length > 0 && (
          <motion.div variants={fadeUp}>
            <Card className="bg-[#070E20]/90 border-cyan-500/30">
              <CardContent className="p-6">
                <h3 className="text-sm font-semibold text-zinc-200 mb-3">Required Skills</h3>
                <div className="flex flex-wrap gap-2">
                  {courseSkills.map(cs => (
                    <span key={cs.skill_id} className="px-3 py-1 rounded-full bg-ink/10 border border-cyan-500/30 text-zinc-200 text-xs">
        ...
        {sessions.length > 0 ? (
          <motion.div variants={fadeUp}>
            <Card className="bg-[#070E20]/90 border-cyan-500/30">
              <CardContent className="p-6">
                <h3 className="text-sm font-semibold text-zinc-200 mb-4">Course Sessions</h3>
                <div className="space-y-3">
                  {sessions.map((session, index) => (
                    <div key={session.id} className="p-3 rounded-xl bg-ink/5 border border-cyan-500/30">
          ...
          <motion.div variants={fadeUp}>
            <Card className="bg-[#070E20]/90 border-cyan-500/30 border-dashed">
              <CardContent className="p-8 flex flex-col items-center justify-center text-center">
                <Calendar className="w-10 h-10 text-zinc-200/20 mb-3" />
                <h3 className="text-sm font-semibold text-zinc-200 mb-1">No Course Sessions</h3>
                <p className="text-xs text-zinc-200/60 mb-4 max-w-xs">Break your course down into topics or schedule live classes.</p>
                <RouterLink to={`/trainer/courses/${courseId}/sessions`}>
                  <Button variant="outline" size="sm" className="border-cyan-500/30 text-zinc-200 bg-transparent hover:bg-ink/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <RouterLink to={`/trainer/courses/${courseId}/sessions`}>
              <Card className="bg-[#070E20]/90 border-cyan-500/30 hover:border-cyan-500/30 transition-all cursor-pointer h-full">
                <CardContent className="p-5 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-ink/10 flex items-center justify-center"><Calendar className="w-5 h-5 text-zinc-200" /></div>
                  <div><p className="text-sm font-medium text-zinc-200">Sessions</p><p className="text-xs text-zinc-200/50">{sessions.length} sessions</p></div>
                </CardContent>
              </Card>
            </RouterLink>
          </motion.div>
          <motion.div variants={fadeUp}>
            <RouterLink to={`/trainer/courses/${courseId}/materials`}>
              <Card className="bg-[#070E20]/90 border-cyan-500/30 hover:border-cyan-500/30 transition-all cursor-pointer h-full">
                <CardContent className="p-5 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-ink/10 flex items-center justify-center"><FileText className="w-5 h-5 text-zinc-200" /></div>
                  <div><p className="text-sm font-medium text-zinc-200">Materials</p><p className="text-xs text-zinc-200/50">{materials.length} files</p></div>
                </CardContent>
              </Card>
            </RouterLink>
          </motion.div>
          ...
          <motion.div variants={fadeUp}>
            <RouterLink to={`/trainer/courses/${courseId}/performance`}>
              <Card className="bg-[#070E20]/90 border-cyan-500/30 hover:border-cyan-500/30 transition-all cursor-pointer h-full">
                <CardContent className="p-5 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-ink/10 flex items-center justify-center"><BarChart3 className="w-5 h-5 text-zinc-200" /></div>
                  <div><p className="text-sm font-medium text-zinc-200">Performance</p><p className="text-xs text-zinc-200/50">{enrollmentCount} trainees</p></div>
                </CardContent>
              </Card>
            </RouterLink>
          </motion.div>
