    ...
    <div className="flex flex-col h-[500px] bg-[#070E20]/90 border border-cyan-500/30 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {msg.is_private && (
                    <span className="flex items-center gap-1 text-[10px] text-rose-400 bg-rose-500/10 px-1.5 py-0.5 rounded">
                      <Lock className="w-3 h-3" /> Private to Trainer
                    </span>
                  )}
                </div>
      ...
      <div className="p-3 bg-cyan-950/20 border-t border-cyan-500/30">
        <div className="flex items-center gap-2 mb-2 px-1">
          {!isTrainer && (
            <label className="flex items-center gap-1.5 cursor-pointer text-xs text-zinc-200/60 hover:text-zinc-200 transition-colors">
              <input 
                type="checkbox" 
                checked={isPrivate} 
                onChange={e => setIsPrivate(e.target.checked)}
                className="rounded border-cyan-500/30 bg-[#070E20]/90 text-purple-600 focus:ring-purple-600"
              />
              <Lock className="w-3 h-3" /> Ask privately
            </label>
          )}
        </div>
        <div className="flex gap-2">
          <Input 
            placeholder={isPrivate ? "Type a private message to the trainer..." : "Type a message to the course..."}
            value={newMessage}
            onChange={e => setNewMessage(e.target.value)}
            onKeyDown={handleKeyDown}
            className="flex-1 bg-[#070E20]/90 border-cyan-500/30 text-zinc-200 focus-visible:ring-purple-600"
          />
          <Button 
