      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div
              ref={scrollRef}
              className="flex-1 p-4 overflow-y-auto bg-slate-50/60 flex flex-col gap-3.5 scroll-smooth"
            >
              {messages.map((msg, i) => (
                <div
                  key={i}
                ...
                >
                  {msg.role === 'bot' ? (
                    ...
                    <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 bg-gradient-to-br from-cyan-600 to-blue-700 text-white shadow-xs">
              ...
              {messages.length <= 1 && (
                <div className="pt-2 flex flex-col gap-1.5">
                  <p className="text-[11px] font-medium text-slate-400 px-1">Suggested questions:</p>
            ...
            >
              <Input
                ...
                className="rounded-xl bg-slate-50/90 border-slate-200 text-slate-900 placeholder:text-slate-400 focus-visible:ring-cyan-500 focus-visible:border-cyan-500 h-10 text-xs sm:text-sm shadow-none"
              />
              <Button
                type="submit"
                size="icon"
                disabled={isLoading || !inputValue.trim()}
                className="rounded-xl h-10 w-10 bg-gradient-to-r from-cyan-600 via-sky-500 to-blue-600 hover:from-cyan-500 hover:to-sky-400 text-white shadow-md shadow-cyan-600/25 shrink-0 cursor-pointer disabled:opacity-40 transition-all"
              >
