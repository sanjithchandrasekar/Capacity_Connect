...
export function Register() {
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex gap-2">
        ...
        {!isEmailVerified && !emailOtpSent && (
          <Button type="button" onClick={handleSendEmailOtp} disabled={isSendingEmailOtp || isLoading} className="h-11 bg-purple-100 text-cyan-300 hover:bg-purple-200">
      ...
      {emailOtpSent && !isEmailVerified && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex gap-2 mt-2">
          <Input 
            ...
            className="h-11 bg-[#070E20]/90 border-pink-500/30"
          />
  ...
  const renderMobileField = (formObj: any) => (
    <div className="space-y-1.5">
      <Label htmlFor="mobileNumber" className="text-zinc-200/80 text-sm font-medium">Mobile Number</Label>
      <div className="flex gap-2">
        <div className="relative shrink-0">
          <select 
            ...
            disabled={isLoading}
            className="h-11 appearance-none bg-purple-500/[0.03] border border-cyan-500/30 rounded-md pl-3 pr-8 text-sm focus:outline-none focus:border-pink-500 cursor-pointer"
          >
            {countryCodes.map(c => (
              <option key={c.code} value={c.code}>{c.flag} {c.code}</option>
            ))}
          </select>
          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-zinc-200/50">
  ...
  if (registrationSuccess) {
    return (
      <div className="min-h-screen bg-[#070E20]/90 text-zinc-200 flex items-center justify-center p-4 md:p-6 py-8 md:py-12 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md relative z-10 text-center bg-[#070E20]/90/90 backdrop-blur-xl border border-cyan-500/30 rounded-2xl p-8 shadow-2xl shadow-cyan-950/50">
    ...
    <div className="min-h-screen bg-[#070E20]/90 text-zinc-200 flex items-center justify-center p-4 md:p-6 py-8 md:py-12 relative overflow-hidden">
      ...
      >
        ...
        <div className="text-center mb-6 md:mb-8">
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold text-zinc-200 tracking-tight">Create Account</h1>
          <p className="text-zinc-200/60 mt-2 text-sm">Join the MoES Capacity Connect platform</p>
        </div>
        ...
        <div className="bg-[#070E20]/90/90 backdrop-blur-xl border border-cyan-500/30 rounded-2xl p-6 md:p-8 shadow-2xl shadow-cyan-950/50">
          <Tabs value={role} onValueChange={(v) => {
            ...
            <TabsList className="grid w-full grid-cols-2 mb-6 p-1 bg-purple-500/5 rounded-xl border border-cyan-500/30">
              <TabsTrigger value="trainee" className="rounded-lg data-[state=active]:bg-[#070E20]/90 data-[state=active]:shadow-sm data-[state=active]:text-cyan-300">Trainee</TabsTrigger>
              <TabsTrigger value="trainer" className="rounded-lg data-[state=active]:bg-[#070E20]/90 data-[state=active]:shadow-sm data-[state=active]:text-cyan-300">Trainer</TabsTrigger>
            </TabsList>

            <TabsContent value="trainee" className="mt-0">
              <form onSubmit={traineeForm.handleSubmit(onSubmit)} className="space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="fullName" className="text-zinc-200/80 text-sm font-medium">Full Name</Label>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="proofFile" className="text-zinc-200/80 text-sm font-medium">Proof Document</Label>
                  ...
                  <p className="text-xs text-zinc-200/50">Please upload your ID or employment proof (PDF, JPG, PNG)</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="department" className="text-zinc-200/80 text-sm font-medium">Department <span className="text-zinc-200/40">(optional)</span></Label>
                    <Input id="department" placeholder="e.g. IMD" {...traineeForm.register('department')} className={inputClass(false)} disabled={isLoading} />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="designation" className="text-zinc-200/80 text-sm font-medium">Designation <span className="text-zinc-200/40">(optional)</span></Label>
            ...
            <TabsContent value="trainer" className="mt-0">
              <form onSubmit={trainerForm.handleSubmit(onSubmit)} className="space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="t-fullName" className="text-zinc-200/80 text-sm font-medium">Full Name</Label>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="studyDetails" className="text-zinc-200/80 text-sm font-medium">Study Details & Qualifications</Label>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="biodataFile" className="text-zinc-200/80 text-sm font-medium">Biodata / Resume</Label>
                  ...
                  <p className="text-xs text-zinc-200/50">Please upload your CV or Biodata (PDF, DOC)</p>
  ...
  return (
    <>
      <div className="flex items-start gap-2 p-3 mt-4 bg-gradient-to-r from-purple-50 via-pink-50 to-orange-50 border border-pink-500/20 rounded-xl">
        ...
        <p className="text-xs text-zinc-200/70 leading-relaxed">
          New accounts require <strong className="text-cyan-300">admin approval</strong> before full access.
        </p>
      </div>
      <Button
        type="submit"
