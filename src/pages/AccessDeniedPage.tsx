    ...
    <div className="min-h-screen bg-cream flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="bg-cream border border-red-200 rounded-2xl p-6 md:p-10 shadow-sm text-center">
          ...
          <h1 className="text-xl md:text-2xl font-bold text-ink mb-3">Access Denied</h1>
          ...
          <div className="space-y-3">
            {session ? (
              <Button onClick={() => navigate('/dashboard')} className="w-full h-10 bg-ink/5 hover:bg-ink/10 text-ink border border-ink/10 transition-all">
                <ArrowLeft className="w-4 h-4 mr-2" /> Go to Dashboard
              </Button>
            ) : (
              <Link to="/login">
                <Button className="w-full h-10 bg-ink/5 hover:bg-ink/10 text-ink border border-ink/10 transition-all">
